...
import { requireRole, fortbildungScope, type SessionUser } from "@/lib/auth";
...
function auswahl(user: SessionUser) {
  return { id: true, name: true, ort: true, strasse: true, schulnummer: true, aktiv: true, istOnline: true, _count: { select: { fortbildungen: { where: fortbildungScope(user) } } } } satisfies Prisma.VeranstaltungsortSelect;
}
...
export async function pruefeSchulimport(_bisher: SchulimportStand, formData: FormData): Promise<SchulimportStand> {
  ...
  const bestand = await prisma.veranstaltungsort.findMany({ select: auswahl(user), orderBy: { id: "asc" } });
...
export async function uebernehmeSchulimport(_bisher: SchulimportStand, formData: FormData): Promise<SchulimportStand> {
  ...
  try {
    const ergebnis = await prisma.$transaction(async (tx) => {
      const bestand = await tx.veranstaltungsort.findMany({ select: auswahl(user), orderBy: { id: "asc" } });
      const plan = planeSchulimport(csv, bestand);
