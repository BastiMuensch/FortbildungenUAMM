...
export async function StartseitenInhalt({ bezirk }: { bezirk?: OeffentlicherBezirk }) {
  ...
  const kommend = { AND: [oeffentlicheFortbildungWhere(bezirk?.id), { ende: { gte: jetzt } }] };
  ...
  const [naechste, anzahlKommend, anzahlSchuljahr, anzahlOnline] = await Promise.all([
    ...
    prisma.fortbildung.count({
      where: {
        AND: [oeffentlicheFortbildungWhere(bezirk?.id), { beginn: { gte: start, lte: ende } }],
      },
    }),
    prisma.fortbildung.count({
    ...
    <div className="space-y-14">
      <section className="rounded-2xl border border-border/80 bg-[linear-gradient(135deg,var(--card)_0%,var(--card)_62%,var(--secondary)_100%)] px-5 py-10 shadow-sm sm:px-10 sm:py-14">
        <div className="max-w-4xl">
          <p className="mb-4 text-sm font-medium text-primary">
            {bezirk ? `Schulamtsbezirk ${bezirk.name}` : schulamt.region}
          </p>
          ...
          <p className="mt-3 max-w-2xl text-base text-muted-foreground">
            {bezirk ? `Fortbildungsangebote für ${schulamt.zielgruppe} im Schulamtsbezirk ${bezirk.name}.` : schulamt.startText}
          </p>

          {/* Einfaches GET-Formular: funktioniert auch ohne JavaScript. */}
          <form action={`${basis}/fortbildungen`} className="mt-7 flex max-w-2xl flex-col gap-2 sm:flex-row sm:gap-0">
          ...
          <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm">
            <Link
              href={`${basis}/fortbildungen?format=ESESSION`}
      ...
      <section>
        ...
        {naechste.length === 0 ? (
          ...
          <div className="grid gap-3">
            {naechste.map((f) => (
              <FortbildungKarte basisPfad={basis} key={f.id} fortbildung={f} maxTn={f.maxTn} />
            ))}
          </div>
        )}
      </section>
      ...
      <section className="rounded-2xl border border-border bg-secondary/55 px-5 py-5 sm:px-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          ...
          <a
            href={bereichsKalenderAbo(bezirk)}
