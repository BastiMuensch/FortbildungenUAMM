import { bereichsPfad, type OeffentlicherBezirk } from "@/lib/oeffentlicherBereich";
...
export async function FortbildungsListenInhalt({
  searchParams, bezirk,
}: {
  ...
  // Das Schulamt kommt verbindlich aus dem Pfad, nicht aus veränderbaren Filtern.
  ...
  const [fortbildungen, schlagworte, bereiche, bezirke] = await Promise.all([
    ...
    prisma.schlagwort.findMany({
      where: { fortbildungen: { some: { fortbildung: oeffentlicheFortbildungWhere(bezirk?.id) } } },
      orderBy: { name: "asc" },
      select: { name: true },
    }),
    ...
    prisma.bezirk.findMany({
      where: { aktiv: true, ...(bezirk ? { id: bezirk.id } : {}) },
      orderBy: { name: "asc" },
    ...
    const wegfall = await Promise.all(
      aktiveFilter.map(async (chip) => {
        ...
        return {
          ...
          href: baueUrl(`${basis}/fortbildungen`, rest, {}),
        };
      }),
    );
    ...
    if (!zeigeVergangene) {
      ...
      if (treffer > 0) {
        rueckblick.push({
          ...
          treffer,
          href: baueUrl(`${basis}/fortbildungen`, params, { vergangene: "1" }),
        });
      }
    }
  ...
  return (
    <div className="space-y-7">
      ...
      {fortbildungen.length === 0 ? (
        <LeerZustand
          auswege={auswege}
          alleZuruecksetzen={`${basis}/fortbildungen`}
          gefiltert={gefiltert}
        />
      ) : (
        <div className="pt-2">
          <MonatsGruppen basisPfad={basis} fortbildungen={fortbildungen} />
        </div>
      )}
    </div>
  );
